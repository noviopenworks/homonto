...
- **Partial (2026-07-13):** F49, F45, F51, F48 (`import-backup-before-overwrite`) archived on `main` — `NoArgs` on
  positional-free commands, and plan/status no longer claim "up to date"/"No drift"
  (exit 0) after an adapter warning. Remaining E2: F46, F50 (`--output json` + exit-code taxonomy), F52.
