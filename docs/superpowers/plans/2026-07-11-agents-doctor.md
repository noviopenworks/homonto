...
- [ ] 2.1 Full regression (build/test/-race/vet/gofmt/mod tidy). E2E (real `homonto`): add local agent→doctor `healthy` exit 0; edit source→doctor drift non-zero; delete installed file→missing-on-disk.
- [ ] 2.2 Update `docs/roadmap.md` v2 status + README (mention `homonto agents doctor`). No over-claim.
