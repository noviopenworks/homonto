...
- [x] Write docs/personas.md: homonto=product, onto=native binary-enforced workflow,
...
- [x] A test that drives the real onto binary through new→advance→set→close and
...
## 3. Verification
- [x] `go test ./...` (incl. the new E2E) -race, vet, build, `openspec validate --all` green.
