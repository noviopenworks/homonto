...
- [x] 2.1 Full regression (build/test/-race/vet/gofmt/mod tidy). E2E (real `homonto`): add a local agent, `agents doctor` → `healthy` exit 0; edit the source file → `agents doctor` reports drift exit non-zero; delete an installed file → reports missing-on-disk.
- [x] 2.2 Update `docs/roadmap.md` v2 status (agents doctor landed) + README (mention `homonto agents doctor`). No over-claim.
